...
\begin{tikzpicture}[
  ...
  box/.style   = {draw=black!55, rounded corners=2pt, align=center,
                  minimum height=7mm, inner xsep=4pt, fill=white},
  op/.style    = {box, fill=blue!6, draw=blue!45},
  keep/.style  = {box, fill=green!8, draw=green!45!black},
  drop/.style  = {box, fill=red!5, draw=red!40},
  ar/.style    = {-{Latex[length=4pt,width=3.6pt]}, draw=black!60},
]

\node[op]   (thr)  at (0,0)      {components of\\[-1pt] $\bar{p}_r > t_{\mathrm{lo}} = 0.25$};
\node[box]  (cand) at (3.05,0)   {4 candidates, peaks\\[-1pt] $0.92$ \; $0.52$ \; $0.35$ \; $0.88$};
\node[op]   (test) at (6.15,0)   {is the peak\\[-1pt] $> t_{\mathrm{hi}} = 0.70$?};
\node[keep] (keep) at (8.70,0.62) {\textbf{2 kept}, whole at\\[-1pt] their $t_{\mathrm{lo}}$ extent};
\node[drop] (drop) at (8.70,-0.62){\textbf{2 deleted}\\[-1pt] entirely};

\draw[ar] (thr)  -- (cand);
\draw[ar] (cand) -- (test);
\draw[ar] (test.east) -- ++(0.25,0) |- (keep.west);
\draw[ar] (test.east) -- ++(0.25,0) |- (drop.west);
\node[font=\scriptsize,anchor=east,inner sep=1.5pt] at (7.32,0.40) {yes};
\node[font=\scriptsize,anchor=east,inner sep=1.5pt] at (7.32,-0.40) {no};

\end{tikzpicture}
\caption{The decode, on a worked example. The two thresholds answer different questions and are
asked in order. $t_{\mathrm{lo}}$ proposes candidates and fixes how far each one extends;
$t_{\mathrm{hi}}$ then decides which of them exist. A candidate whose peak falls between the two
thresholds is deleted in full, however large it is, because nothing inside it ever clears
$t_{\mathrm{hi}}$; a candidate that clears it is kept in full, however small it is. Neither width
nor mean probability is used at any point, which is what separates this rule from the component
filters of Section~\ref{sec:fp}. The four peaks are an example for illustration, not measured
values: our residual false positives peak at $0.96$ and would pass $t_{\mathrm{hi}}$, which is why
