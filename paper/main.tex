...
is unpromising here and identifies where the remaining headroom is not. Finally, we report a
validation failure that any team tuning post-processing on models trained without a held-out fold
should expect: a cavity keep-rule measured at $+0.028$ lesion-wise Dice on our training-case
harness delivered $-0.016$ on held-out validation, the wrong sign, because a memorised model
rarely emits the spurious components such a rule would admit.
...
\item \textbf{A validation warning for lesion-wise post-processing.} Tuning post-processing on
      cases the models were trained on is systematically optimistic, and for one component
      keep-rule it reversed sign on held-out validation. We give the mechanism and quantify the
      cost (Section~\ref{sec:harness}).
\end{enumerate}
...
blob loss~\cite{kofler2022blob,kundu2026instance}. We claim the empirical finding that this
combination is where the performance is under lesion-wise scoring, together with the error
analysis and the validation warning that follow from it.
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[xscale=0.78, yscale=2.70]

% ---- threshold band --------------------------------------------------------
\fill[blue!5]  (0,0.25) rectangle (9.55,0.70);

% ---- axes ------------------------------------------------------------------
\draw[->,gray!70] (0,0) -- (9.95,0) node[right,black,font=\scriptsize,xshift=1pt] {position};
...
\node[rotate=90,font=\scriptsize,anchor=south] at (-0.72,0.5) {$\bar{p}_r(x)$};

% ---- thresholds (labelled to the right, clear of the profile) --------------
\draw[densely dashed,red!70!black]  (0,0.70) -- (9.55,0.70);
\draw[densely dashed,blue!70!black] (0,0.25) -- (9.55,0.25);
\node[font=\scriptsize,red!60!black,anchor=west]
  at (9.70,0.70) {$t_{\mathrm{hi}}=0.70$ \,(existence)};
\node[font=\scriptsize,blue!60!black,anchor=west]
  at (9.70,0.25) {$t_{\mathrm{lo}}=0.25$ \,(extent)};
...
% ---- component names -------------------------------------------------------
\node[font=\scriptsize,anchor=south] at (2.00,0.95) {large lesion};
\node[font=\scriptsize,anchor=south] at (4.55,0.56) {wide, low peak};
\node[font=\scriptsize,anchor=south] at (6.60,0.38) {narrow, low peak};
\node[font=\scriptsize,anchor=south] at (8.60,0.91) {small lesion};

% ---- resulting masks, one strip per rule -----------------------------------
% strip 1: threshold at t_hi only -- correct rejections, but both lesions truncated
\fill[black!45] (1.69,-0.19) rectangle (2.31,-0.13);
\fill[black!45] (8.50,-0.19) rectangle (8.70,-0.13);
\node[font=\scriptsize,black!60,anchor=west] at (9.70,-0.16)
  {$t_{\mathrm{hi}}$ only: lesions cut short};

% strip 2: threshold at t_lo only -- correct extents, but the junk comes too
\fill[red!65!black] (1.20,-0.37) rectangle (2.80,-0.31);
\fill[red!65!black] (3.75,-0.37) rectangle (5.35,-0.31);
\fill[red!65!black] (6.30,-0.37) rectangle (6.90,-0.31);
\fill[red!65!black] (8.30,-0.37) rectangle (8.90,-0.31);
\node[font=\scriptsize,red!55!black,anchor=west] at (9.70,-0.34)
  {$t_{\mathrm{lo}}$ only: low peaks kept};

% strip 3: hysteresis -- extents of strip 2, existence test of strip 1
\fill[green!45!black] (1.20,-0.55) rectangle (2.80,-0.49);
\fill[green!45!black] (8.30,-0.55) rectangle (8.90,-0.49);
\node[font=\scriptsize,green!35!black,anchor=west] at (9.70,-0.52)
  {\textbf{hysteresis}: neither problem};

\end{tikzpicture}
\caption{Hysteresis decoding on a 1-D slice through the fused probability map. The four components
are drawn to cross two widths with two peak heights; they are illustrative, not measured. The
three strips are the masks each rule produces. A single strict threshold (grey) removes both
low-peak components but keeps only the tips of the two lesions. A single permissive threshold
(red) recovers the full extent of the lesions but keeps the low-peak components too. Hysteresis
(green) takes the extents of the permissive mask and the existence test of the strict one: a
component is kept only if it contains a voxel above $t_{\mathrm{hi}}$, and is then kept
\emph{whole}. Both low-peak components lie entirely inside the shaded band, so neither contains
such a voxel and both go, whatever their width; the small lesion is drawn as narrow as one of them
and is kept on its peak alone. Width and mean probability are never used. Note that this is not
how our residual false positives behave: they peak at 0.96 and would pass $t_{\mathrm{hi}}$, which
is why Section~\ref{sec:fp} is needed.}
\label{fig:hyst}
\end{figure}
...
We also determined that ``ResEnc-XL'' and ``ResEnc-L'' instantiate an identical network for this
dataset, the planners differing only in memory target, so architecture scaling is not a
decorrelation lever here.
...
in it, and all three failed for that shared reason. Because they span the decode, the loss and
the threshold, the failure is unlikely to be an artefact of one implementation. We do not claim
more than that: seven methods on one system cannot rule out every confidence-aware approach, and
the peak-probability comparison above is itself conditioned on the decoding threshold, so it
shows that raising the threshold will not fix both error types rather than that confidence is
inverted in general. We report them in full because several are standard practice a reader would
otherwise reasonably invest in. Two mechanisms are instructive: the per-component loss penalty collapsed into global
...
and threshold levels, which is what makes it a property of the ranking quantity on this system
rather than of any one implementation.}
...
\subsection{The false positives are consistent with anatomical mimics}
...
Visual review of the 48 most confident false positives is more consistent with the mimic
hypothesis than with the annotation-gap one, though it does not settle between them. They are
...
hypothesis to test rather than a clinical assessment. Anatomical position is a genuinely different
separation axis from confidence, so we measured it. Distance from the brain surface to the
false-positive centroid has a median of 8.8\,mm against 11.2\,mm for reference lesion centroids,
with almost fully overlapping interquartile ranges and an area under the ROC curve of
\textbf{0.575}. False positives are only marginally more peripheral, and grey--white junction
metastases, a common true presentation, sit at the same depth, so any filter aggressive enough to
remove the mimics removes real lesions with them. The measurement is worth reporting because the
hypothesis is attractive: making that route work would take a discriminant far sharper than raw
surface proximity.
...
\subsection{A validation protocol for lesion-wise post-processing}
\label{sec:harness}
...
submission cost. It also revealed a boundary condition that applies to any team tuning post-processing on models
trained without a held-out fold, and which we state here as a protocol.

Every model was trained on these cases. For decodes (the same probabilities post-processed
differently) this is least damaging, because memorisation is common-mode and largely cancels from
a \emph{ranking} of candidates. It is worst for a rule whose whole effect is to admit components
the model would otherwise not have emitted: on memorised cases it rarely emits a spurious
component, so a ``keep more components'' rule appears free, while on unseen data the same rule
admits them. \textbf{Memorisation suppresses precisely the false positives that the rule would
let in, so the harness is structurally blind to its cost.}
...
We state this as an empirical warning, not a rule. It is tempting to say thresholds and fusion
weights are safe offline because they change values rather than counts, but they too create,
remove, split and merge components, and lesion-wise scoring charges for every one. What we can
report is narrower: our harness was not an unbiased validation set; rankings still transferred
acceptably for the threshold and fusion settings we tested, which made it useful; and one
count-expanding keep-rule reversed sign. Treat every offline decision as provisional, and be most
suspicious of rules admitting components the model would not otherwise emit.
...
strategies failed to address it; the false positives are consistent with anatomical mimics but not
separable by anatomical position (AUC 0.575); and lesion-wise Dice actively penalises recovering
additional small lesions. We therefore contribute, alongside the method, an error analysis that
narrows where future effort is likely to be repaid and a warning about which offline
post-processing experiments transfer. All code is released at \url{https://github.com/aymuos15/brats2026-mets}.
