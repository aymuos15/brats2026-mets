...
% TODO(authors): real names, affiliations, ORCIDs, emails before submission.
\titlerunning{Confident Mistakes in Brain Metastasis Segmentation}

\author{Anonymous Author\inst{1}}
\authorrunning{Anonymous et al.}
\institute{School of Biomedical Engineering \& Imaging Sciences,\\ King's College London, London, UK\\
\email{TODO@kcl.ac.uk}}
...
\emph{lesion-wise}~\cite{bratsmet2023}. Predicted components are matched to reference
components, each lesion contributes equally to the Dice and normalised surface Dice (NSD)
...
blob loss~\cite{kofler2022blob}. We claim the empirical finding that this combination is where
...
lesion equally rather than each voxel, following the blob-loss principle~\cite{kofler2022blob}.
