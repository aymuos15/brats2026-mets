...
% ---- what a single strict threshold would have kept ------------------------
\draw[line width=2.6pt,black!45] (1.69,-0.10) -- (2.31,-0.10);
\node[font=\scriptsize,black!55,anchor=west] at (2.45,-0.10)
  {extent kept by $t_{\mathrm{hi}}$ alone: the margin is amputated};

% ---- extents retained / dropped by hysteresis ------------------------------
\draw[line width=2.6pt,green!45!black]            (1.20,-0.24) -- (2.80,-0.24);
\draw[line width=2.6pt,red!65!black] (3.75,-0.24) -- (5.35,-0.24);
\draw[line width=2.6pt,red!65!black] (6.30,-0.24) -- (6.90,-0.24);
\draw[line width=2.6pt,green!45!black]            (8.30,-0.24) -- (8.90,-0.24);

% ---- verdicts --------------------------------------------------------------
\node[font=\scriptsize,align=center,anchor=north,green!35!black] at (2.00,-0.29)
  {\textbf{kept}\\[-1pt] peak $0.92$};
\node[font=\scriptsize,align=center,anchor=north,red!55!black]   at (4.55,-0.29)
  {\textbf{dropped}\\[-1pt] peak $0.52$};
\node[font=\scriptsize,align=center,anchor=north,red!55!black]   at (6.60,-0.29)
  {\textbf{dropped}\\[-1pt] peak $0.35$};
\node[font=\scriptsize,align=center,anchor=north,green!35!black] at (8.60,-0.29)
  {\textbf{kept}\\[-1pt] peak $0.88$};
...
\end{tikzpicture}
\caption{Hysteresis decoding on a 1-D slice through the fused probability map. Every component of
$\bar{p}_r>t_{\mathrm{lo}}$ is a candidate; it is kept \emph{in full} if its peak clears
$t_{\mathrm{hi}}$ (green) and deleted entirely if it never does (red). The grey bar shows what a
single strict threshold at $t_{\mathrm{hi}}$ would have kept of the large lesion: everything in
the shaded band is amputated, which is the margin the second threshold exists to recover. The
noise speck and the small lesion are drawn the same width and meet opposite fates, decided only
by peak; size and mean probability are never consulted, which is what separates this rule from
the component filters of Section~\ref{sec:fp}.}
\label{fig:hyst}
\end{figure}
