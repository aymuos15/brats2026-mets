...
% ---- threshold bands -------------------------------------------------------
\fill[blue!5]  (0,0.25) rectangle (9.6,0.70);

% ---- axes ------------------------------------------------------------------
\draw[->,gray!70] (0,0) -- (9.9,0) node[right,black,font=\scriptsize,xshift=1pt] {position};
...
\draw[densely dashed,red!70!black]  (0,0.70) -- (9.6,0.70);
\draw[densely dashed,blue!70!black] (0,0.25) -- (9.6,0.25);
\node[font=\scriptsize,red!60!black,anchor=west]
  at (9.75,0.70) {$t_{\mathrm{hi}}=0.70$ \,(existence)};
\node[font=\scriptsize,blue!60!black,anchor=west]
  at (9.75,0.25) {$t_{\mathrm{lo}}=0.25$ \,(extent)};
...
spans $t_{\mathrm{lo}}$ to $t_{\mathrm{hi}}$. Thresholding at $t_{\mathrm{hi}}$ alone would keep
only the peaks above the band, amputating each lesion's margin; at $t_{\mathrm{lo}}$ alone it
would admit the diffuse region. Hysteresis keeps every component of
$\bar{p}_r>t_{\mathrm{lo}}$ whose peak clears $t_{\mathrm{hi}}$, at full $t_{\mathrm{lo}}$-extent
(green), and drops those that never do (red). Size is never consulted, so the 30-voxel lesion survives on one
...
intervals. The challenge server returns only aggregate scores, withholding per-case metrics and
reference labels, so dispersion and paired significance tests cannot be computed there at all;
where we control the evaluation we report per-case counts instead. The aggregates are
deterministic, so differences between them are exact, but we cannot say how much of a small
difference would survive a different cohort.
