...
\node[font=\scriptsize,anchor=south] at (4.55,0.56) {diffuse region};
\node[font=\scriptsize,anchor=south] at (6.60,0.38) {noise speck};
...
\node[font=\scriptsize,black!60,anchor=west] at (9.70,-0.16)
  {$t_{\mathrm{hi}}$ only: margins amputated};
...
\node[font=\scriptsize,red!55!black,anchor=west] at (9.70,-0.34)
  {$t_{\mathrm{lo}}$ only: junk admitted};
...
\node[font=\scriptsize,green!35!black,anchor=west] at (9.70,-0.52)
  {\textbf{hysteresis}: both right};

\end{tikzpicture}
\caption{Hysteresis decoding on a 1-D slice through the fused probability map. The three strips
are the masks each rule produces. A single strict threshold (grey) rejects the junk but keeps only
the tips of the two lesions. A single permissive threshold (red) recovers their full extent but
admits the diffuse region and the speck as well. Hysteresis (green) takes the extents of the
permissive mask and the existence test of the strict one: a component survives only if it contains
a voxel above $t_{\mathrm{hi}}$, and then survives \emph{whole}. The diffuse region and the speck
lie entirely inside the shaded band, so neither contains such a voxel and both vanish, however wide
they are; the small lesion is drawn the same width as the speck and survives on its peak alone.
Size and mean probability are never consulted, which is what separates this rule from the
component filters of Section~\ref{sec:fp}.}
\label{fig:hyst}
\end{figure}
