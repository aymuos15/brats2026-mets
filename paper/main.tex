...
\title{Inference-Time Decoding for Brain Metastasis\\ Segmentation: Ensemble Dilution, Hysteresis\\ Thresholding, and Safe Post-processing Validation}
...
component size. Second, we characterise the residual error and derive a design rule from it: the median peak
probability inside our false positives is 0.96 against 0.55 inside the lesions we miss, so
confidence is inverted with respect to correctness, and any criterion monotone in confidence will
remove true lesions before false ones. We establish this across seven suppression strategies
spanning the decode, the loss and the seed threshold, which identifies where the remaining
headroom is not. Third, we give a validation protocol for lesion-wise post-processing: rules that
change how many components are retained cannot be tuned on training cases even relatively,
because a memorised model rarely emits the spurious components such a rule would admit, an effect
we quantify at $+0.028$ Dice offline against $-0.016$ measured. Together the second and third
contributions tell practitioners which post-processing families are worth pursuing and which
offline experiments can be trusted to predict held-out behaviour.
...
\item \textbf{A diagnosis of the residual error, and a design rule that follows from it.} The
      remaining false positives are confident, coherent, isolated and corroborated across
      independently trained networks. We establish across seven suppression strategies that any
      criterion monotone in confidence is ruled out for this task, which narrows the search space
      for future work (Section~\ref{sec:fp}).
\item \textbf{A validation protocol for lesion-wise post-processing.} We identify exactly which
      post-processing can be tuned offline on models trained without a held-out fold and which
      cannot, give the mechanism that separates the two cases, and quantify the cost of getting
      it wrong (Section~\ref{sec:harness}).
\end{enumerate}
...
the performance is under lesion-wise scoring, together with the design rule and the validation
protocol that follow from our error analysis. Readers looking for a new architecture will not
find one, and our argument is that they should not be looking.
...
We evaluated seven suppression strategies against this (Table~\ref{tab:suppression}). They differ in
where they intervene (after decoding, inside the loss, or at the seed threshold), but all three
families reduce to the same operation, ranking components by confidence or by a quantity monotone
in it, and all three fail for that shared reason. The breadth matters: because the families span
the decode, the loss and the threshold, the result is not a property of one implementation but of
the ranking quantity itself, which is what licenses the design rule. We report them in full because several are standard practice a reader would
otherwise reasonably invest in, and knowing which branches are closed is the practical value
here. Two mechanisms are instructive: the per-component loss penalty collapsed into global
...
and our false positives sit at the confident end. The shared failure is what establishes the
design rule, and the coverage across decode, loss and threshold levels is what makes it general.}
...
\subsection{The false positives are anatomical mimics}
...
removes real lesions with them. The measurement is worth reporting precisely because the
hypothesis is attractive: anatomical context is the obvious thing a radiologist uses and a network
lacks, and we can now say what it would take for that route to work, namely a discriminant far
sharper than raw surface proximity.
...
roughly a factor of three. The protocol that follows is simple and, we believe, generally applicable: thresholds and fusion
weights may be tuned offline, because they change values rather than counts; component filters,
keep-rules and size cutoffs may not, and must be validated on a real submission or a genuine
held-out fold. Applying it is what let us spend our remaining submissions on decisions the
harness could actually settle.
...
recovering additional small lesions. We therefore contribute, alongside the method, a design rule
that narrows where future effort should go and a protocol for deciding which post-processing can
be trusted to transfer. All code is released at \url{https://github.com/aymuos15/brats2026-mets}.
