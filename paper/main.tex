...
This one fact refutes seven suppression mechanisms (Table~\ref{tab:refuted}). They differ in
...
in it, which is why they fail together. We report them because the failure is
...
\begin{table}[t]
\centering
\caption{The seven refuted false-positive suppression mechanisms, grouped by the quantity they
rank components on. All three families reduce to ordering components by confidence, or by
something monotone in it, and our false positives sit at the confident end.}
\label{tab:refuted}
\small
...
mechanism & outcome \\
\midrule
\multicolumn{2}{@{}l}{\emph{(a) Post-hoc component filters}: rank on size or mean confidence}\\
\addlinespace[2pt]
...
\midrule
\multicolumn{2}{@{}l}{\emph{(b) Loss-level suppression}: rank on a learned per-component score}\\
\addlinespace[2pt]
...
\midrule
\multicolumn{2}{@{}l}{\emph{(c) Seed-threshold variants}: rank on a transformed confidence}\\
\addlinespace[2pt]
