...
for hi in [0.65, 0.70, 0.75]:
    V.append((f"fuse4eq .30/{hi:.2f}",  "mean4",   "hyst", 0.30, hi, None))
    V.append((f"fuse4sc5 .30/{hi:.2f}", "f4_0.50", "hyst", 0.30, hi, None))
VARIANTS = V
...
def bases(cid):
    ...
    sc = np.load(f"{LE}/pred_smallcc/{cid}.npz")["probabilities"][:3]
    d = {"mean_tc": 0.5 * pt + 0.5 * cp,
         "mean_cct": 0.5 * cc + 0.5 * pt,
         "mean3": (pt + cp + cc) / 3.0,
         "mean4": (pt + cp + cc + sc) / 4.0,
         "f4_0.50": (pt + cp + cc + 0.5 * sc) / 3.5}
    for w in (0.25, 0.35):
...
if __name__ == "__main__":
    ...
    print("\n=== vs LOCAL anchors (WIN 3way .30/.70 = 9771508 CURRENT BEST) ===")
    ref = rows["WIN 3way hyst.30/.70"]
